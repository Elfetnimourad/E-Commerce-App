...
describe("ProductList component", () => {
  ...
  test("renders product list correctly", async () => {
    ...
    const product3 = await screen.findAllByRole("button", { name: "Buy Here" });

    const product4 = await screen.findAllByText("men's clothing");

    // Assert that the products are rendered
    expect(product1).toBeInTheDocument();
    expect(product2).toBeInTheDocument();
    expect(product3).toHaveLength(20);
    expect(product4).toHaveLength(4);

    // Assert that the "Buy Here" button is rendered for each product
  });
